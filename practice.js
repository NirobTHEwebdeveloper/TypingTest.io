...
inputfield = document.querySelector(".Content .input-field")
timeTag = document.querySelector(".time span b")
mistakeTag = document.querySelector(".mistakes span")
wpmTag = document.querySelector(".wpm span")
cpmTag = document.querySelector(".cpm span")
tryAgainBtn = document.querySelector(".content1 button")
...
function randomparagraph (){
    ...
    paragraphs[randomindex].split("").forEach(span =>{
        ...
        let spnatag = `<span>${span}</span>`
        typingtest.innerHTML += spnatag;
    });
    typingtest.querySelectorAll("span")[0].classList.add("active");
    //Focusing input field in keydown section
...
function initTyping (){
    const charachters = typingtest.querySelectorAll("span")
    ...
    if(charIndex < charachters.length - 1 && timeLeft > 0){
        ...
        if(charachters [charIndex].innerText === typechar){
        ...
        cpmTag.innerText = charIndex - mistakes // cpm will not count as mistakes
    }  else {
        inpField.value = "";
        clearInterval(timer);
